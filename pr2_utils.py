...
def compute_stereo(path_l, path_r, img_l, img_r):

  image_l = cv2.imread(os.path.join(path_l,img_l),0)
  image_r = cv2.imread(os.path.join(path_r,img_r),0)
...
def show_lidar(i):
  _, lidar_data = read_data_from_csv('code/data/sensor_data/lidar.csv')
  angles = np.linspace(-5, 185, 286) / 180 * np.pi
  ranges = lidar_data[i, :]
  plt.figure()
